...
public class MyInterceptor {
	...
	@Before("anyMethod()&& args(userName)") // 定义前置通知 执行业务方法前执行 args(userName)
											// 表示要执行的方式必须为一个参数并为Person类型.这样就给他再加了限制条件
	public void doAccessCheck(Person userName) {
		System.out.println(userName);// 得到输入的参数
		System.out.println("执行前置通知");
	}

	@AfterReturning(pointcut = "anyMethod()") // 定义后置通知 执行完业务方法后执行
												// 如果例外通知执行，那么它不会执行
